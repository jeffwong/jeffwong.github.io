...
The variance of $y$ is broken down using the 
...
of whether or not the unit actually received the treatment. This is a homoskedasticity assumption.

The second term is the variance of a random variable that has two parts, which is
a scaled bernoulli. The inner part is  
$E[y | X = 0] =\mu_0$ and $E[y | X = 1] = \mu_1 = \mu_0 + \Delta$.
We leverage the property of scaled bernoullis that can take value $\mu_0$ and $\mu_1$
with probabilities $(1-p_1)$ and $p_1$.

$$Var(E[y | X]) = p_1 (1 - p_1) \Delta^2.$$

We have the key pieces we need to express statistical power in terms of variables
that we can plan for, namely the assignment $Z$ and approximations for the compliance rate.
...
Using some simple numbers, if $p_1 = p_0 = 0.9$, then $\delta' = 0.8 \delta$. 
Referring back to our MDE calculator, by changing the effect size by 0.8,
we change the sample size necessary by $\frac{1}{.8^2} = 56\%$!
